...
State Game::GetInitialState() {
  ...
  if (false) {
    ...
    // Kings
    board.SetField(1, 1, make_piece(Figure::kKing, Player::kWhite));
    board.SetField(3, 3, make_piece(Figure::kKing, Player::kBlack));

    board.SetField(0, 0, make_piece(Figure::kKnight, Player::kWhite));
  ...
  if (true) {
    ...
    board.SetField(3, 7, make_piece(Figure::kQueen, Player::kBlack));
    board.SetField(4, 7, make_piece(Figure::kBishop, Player::kBlack));
    board.SetField(6, 7, make_piece(Figure::kRook, Player::kBlack));
    board.SetField(7, 7, make_piece(Figure::kKing, Player::kBlack));
    board.SetField(0, 6, make_piece(Figure::kPawn, Player::kBlack));
    board.SetField(5, 6, make_piece(Figure::kPawn, Player::kBlack));
    board.SetField(7, 6, make_piece(Figure::kPawn, Player::kBlack));
    board.SetField(2, 5, make_piece(Figure::kKnight, Player::kBlack));
    board.SetField(4, 5, make_piece(Figure::kPawn, Player::kBlack));
    board.SetField(6, 5, make_piece(Figure::kPawn, Player::kBlack));
    board.SetField(5, 5, make_piece(Figure::kPawn, Player::kWhite));
    board.SetField(0, 4, make_piece(Figure::kBishop, Player::kWhite));
    board.SetField(4, 4, make_piece(Figure::kKnight, Player::kWhite));
    board.SetField(2, 3, make_piece(Figure::kRook, Player::kWhite));
    board.SetField(4, 3, make_piece(Figure::kPawn, Player::kWhite));
    board.SetField(7, 3, make_piece(Figure::kQueen, Player::kWhite));
    board.SetField(2, 2, make_piece(Figure::kPawn, Player::kWhite));
    board.SetField(5, 2, make_piece(Figure::kRook, Player::kWhite));
    board.SetField(1, 1, make_piece(Figure::kPawn, Player::kWhite));
...
bool Game::IsTerminalState(State& state) {
  // Check for a draw
  if (state.GetNoProgressCount() > 50) return true;

  if (state.GetMoveCount() > 120) return true;
...
int Game::GetStateResult(State& state) {
  ...
  if (GetLegalActions(state).size() == 0) {
    if (KingInCheck(&state, state.GetPlayer())) {
      if (state.GetPlayer() == Player::kBlack)
        return 1;
      else
        return -1;
    }
...
bool Game::SearchForLineAttack(State* state, unsigned x, unsigned y,
                               BoardPlane* filtered_enemies, unsigned bb_offset,
                               unsigned inv_bb, bool lower,
  ...
  signed first_enemy_in_path =
      lower ? (line_of_attack & enemy_figures).msb()
            : (line_of_attack & enemy_figures).find_first();

  BoardPlane allies_in_path = line_of_attack & player_figures;
  signed first_ally_in_path =
      lower ? allies_in_path.msb() : allies_in_path.find_first();
  signed second_ally_in_path = lower ? allies_in_path.prev(first_ally_in_path)
  ...
  if (first_enemy_in_path < 0 ||
    ...
    if (second_ally_in_path >= 0 &&
        (lower ? second_ally_in_path > first_enemy_in_path
               : second_ally_in_path < first_enemy_in_path))
      // blocked by two allies, no pinned pieces
      return false;
    if (pinned) {
      *pinned |= line_of_attack &
                 magic_bit_planes_[first_enemy_in_path * bb_per_pos + inv_bb];
...
unsigned Game::SearchForKnightAttack(State* state, unsigned x, unsigned y,
  ...
  if (constrained_to) *constrained_to |= attacking_knights;
...
unsigned Game::GeneratePositionAttackers(State* state, unsigned x, unsigned y,
  ...
  for (unsigned rook_upper_bb = 0; rook_upper_bb < 2; ++rook_upper_bb) {
    attacker_count +=
        SearchForLineAttack(state, x, y, &enemy_rook_queen, rook_upper_bb,
                            rook_upper_bb + 2, false, constrained_to, pinned);
  }

  // Get rook-like attackers attacking from lower part
  for (unsigned rook_lower_bb = 0; rook_lower_bb < 2; ++rook_lower_bb) {
    attacker_count +=
        SearchForLineAttack(state, x, y, &enemy_rook_queen, rook_lower_bb + 2,
                            rook_lower_bb, true, constrained_to, pinned);
  }

  // Get position of enemy pieces that can move like a bishop
  BoardPlane enemy_bishop_queen =
      current_board.GetPlayerFigurePlane(
          enemy, static_cast<unsigned>(Figure::kBishop)) |
      current_board.GetPlayerFigurePlane(enemy,
                                         static_cast<unsigned>(Figure::kQueen));

  // Get bishop-like attackers attacking from upper part
  for (unsigned bishop_upper_bb = 0; bishop_upper_bb < 2; ++bishop_upper_bb) {
    attacker_count += SearchForLineAttack(
        state, x, y, &enemy_bishop_queen, bishop_upper_bb + 4,
        bishop_upper_bb + 6, false, constrained_to, pinned);
  }

  // Get bishop-like attackers attacking from lower part
  for (unsigned bishop_lower_bb = 0; bishop_lower_bb < 2; ++bishop_lower_bb) {
    attacker_count += SearchForLineAttack(
        state, x, y, &enemy_bishop_queen, bishop_lower_bb + 6,
        bishop_lower_bb + 4, true, constrained_to, pinned);
  }
...
void Game::GenerateMoveFromBB(State* state, unsigned x, unsigned y,
                              std::vector<State>* moves, unsigned bb_offset,
                              unsigned inv_bb, bool lower, bool rook,
  ...
  BoardPlane player_figures = current_board.GetPlayerPlane(current_player);
  BoardPlane enemy_figures = current_board.GetPlayerPlane(enemy);
  BoardPlane move_bb = magic_bit_planes_[arr_pos * bb_per_pos + bb_offset];

  if (!constrained_to.empty()) move_bb &= constrained_to;
  ...
  if (first_ally_in_path < 0) {
    ...
    if (first_enemy_in_path < 0) {
      ...
      if (lower ? first_ally_in_path > first_enemy_in_path
        ...
        possible_moves |=
            move_bb &
            magic_bit_planes_[first_ally_in_path * bb_per_pos + inv_bb];
      } else {
        possible_moves |=
            move_bb &
            magic_bit_planes_[first_enemy_in_path * bb_per_pos + inv_bb];
  ...
  while (movable_field >= 0) {
    ...
    if (movable_field / width == y + 2 * direction) {
      ...
      for (Figure figure : Game::figures) {
        ...
        moves->back().GetBoard().SetField(
            movable_field % width, movable_field / width,
...
void Game::GenRookMoves(State* state, unsigned x, unsigned y,
  ...
  GenerateMoveFromBB(state, x, y, moves, 0, 2, false, true, constrained_to,
                     pinned);

  // Generate moves in direction up
  GenerateMoveFromBB(state, x, y, moves, 1, 3, false, true, constrained_to,
                     pinned);

  // Generate moves in direction left
  GenerateMoveFromBB(state, x, y, moves, 2, 0, true, true, constrained_to,
                     pinned);

  // Generate moves in direction down
  GenerateMoveFromBB(state, x, y, moves, 3, 1, true, true, constrained_to,
                     pinned);
}
...
void Game::GenBishopMoves(State* state, unsigned x, unsigned y,
  ...
  GenerateMoveFromBB(state, x, y, moves, 4, 6, false, false, constrained_to,
                     pinned);

  // Generate moves in direction upright
  GenerateMoveFromBB(state, x, y, moves, 5, 7, false, false, constrained_to,
                     pinned);

  // Generate moves in direction downright
  GenerateMoveFromBB(state, x, y, moves, 6, 4, true, false, constrained_to,
                     pinned);

  // Generate moves in direction downleft
  GenerateMoveFromBB(state, x, y, moves, 7, 5, true, false, constrained_to,
                     pinned);
}

void Game::GenKingMoves(State* state, unsigned x, unsigned y,
                        std::vector<State>* moves,
                        const BoardPlane& attacked_fields) {
  signed width = GetOption("board_width");
  signed height = GetOption("board_height");
  Board& current_board = state->GetBoard();
  Player current_player = state->GetPlayer();
...
void Game::GenQueenMoves(State* state, unsigned x, unsigned y,
                         ...
                         const BoardPlane& pinned) {
  // Generate moves in direction right
  GenerateMoveFromBB(state, x, y, moves, 0, 2, false, false, constrained_to,
                     pinned);

  // Generate moves in direction up
  GenerateMoveFromBB(state, x, y, moves, 1, 3, false, false, constrained_to,
                     pinned);

  // Generate moves in direction left
  GenerateMoveFromBB(state, x, y, moves, 2, 0, true, false, constrained_to,
                     pinned);

  // Generate moves in direction down
  GenerateMoveFromBB(state, x, y, moves, 3, 1, true, false, constrained_to,
                     pinned);
...
std::vector<State> Game::GenMoves(State* state, bool pseudo) {
  ...
  if (attackers > 1) {
    std::for_each(moves.begin(), moves.end(), [state, enemy](State& s) {
      s.IncMoveCount();
      s.SetPlayer(enemy);
      s.SetDPushPawnX(-1);
  ...
  signed queen_pos = state->GetBoard()
                         .GetPlayerFigurePlane(current_player, Figure::kQueen)
                         .find_first();
  ...
  while (next_pos >= 0) {
    GenPawnMoves(state, next_pos % width, next_pos / width, &moves, constrained,
                 pinned);
    next_pos = pawn.next(next_pos);
  }

  std::for_each(moves.begin(), moves.end(), [&enemy](State& s) {
    s.IncMoveCount();
    s.SetPlayer(enemy);
  });
  return moves;
...
std::vector<State> Game::GenMoves(State* state, unsigned x, unsigned y,
  ...
  BoardPlane constraint = pseudo ? BoardPlane{1, 1} : BoardPlane{width, height};

  BoardPlane pinned = pseudo ? BoardPlane{1, 1} : BoardPlane{width, height};
  // Make checks

  if (!pseudo) {
    unsigned king_pos =
        state->GetBoard()
            .GetPlayerFigurePlane(state->GetPlayer(), Figure::kKing)
            .find_first();
    GeneratePositionAttackers(state, king_pos % width, king_pos / width,
                              &constraint, &pinned);
  }

  if (piece.player != state->GetPlayer() || piece == kEmptyPiece) return moves;
  ...
  std::for_each(moves.begin(), moves.end(), [&next_player](State& s) {
    s.IncMoveCount();
