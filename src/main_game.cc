...
void Benchmark(Game game) {
  aithena::MCTSNode<aithena::chess::Game> root(
      std::make_shared<aithena::chess::Game>(game), game.GetInitialState(),
      nullptr);
  aithena::MCTS<aithena::chess::Game> mcts{
      std::make_shared<aithena::chess::Game>(game)};
  auto root_ptr =
...
State EvaluateUserInput(const std::string input, Game& game, State& state) {
  ...
  if (input_parts[0].compare("start") == 0) {
    ...
    if (piece.figure == static_cast<unsigned>(aithena::chess::Figure::kPawn)) {
      if (static_cast<unsigned>(state.GetDPushPawnX()) == std::get<0>(target) &&
          (static_cast<unsigned>(state.GetDPushPawnY() % 4) ==
           std::get<1>(target))) {
        move_board.ClearField(std::get<0>(target),
                              static_cast<unsigned>(state.GetDPushPawnY() / 4));
      }
    }
    ...
    aithena::MCTSNode<aithena::chess::Game> root(
        std::make_shared<aithena::chess::Game>(game), state,
        static_cast<aithena::MCTSNode<aithena::chess::Game>::NodePtr>(nullptr));
    aithena::MCTS<aithena::chess::Game> mcts{
        std::make_shared<aithena::chess::Game>(game)};
    auto root_ptr =
        std::make_shared<aithena::MCTSNode<aithena::chess::Game>>(root);
