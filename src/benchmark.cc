...
int RunBenchmark(int argc, char** argv) {
  ...
  double nps = 1000000.0 * static_cast<double>(nodes) /
               static_cast<double>(bm_perft.GetLast(Benchmark::UNIT_USEC));

  bm_bm.End();

  std::cout << "## Benchmark (completed in "
            << bm_bm.GetLast(Benchmark::UNIT_SEC) << " seconds) ##"
            << std::endl;
  std::cout << "Perft(" << perft_depth << "): Searched " << nodes
            << " nodes in " << bm_perft.GetLast(Benchmark::UNIT_SEC)
            << " seconds (" << nps << " nps)" << std::endl;
