...
TEST_F(ChessPerftTest, Custom3) {
  auto state = chess::State::FromFEN("r1bqkbnr/pppppppp/n7/8/8/P7/1PPPPPPP/RNBQKBNR w KQkq - 2 2");

  EXPECT_EQ(perft(game_, state, 1), 19);
}

TEST_F(ChessPerftTest, Custom4) {
  auto state = chess::State::FromFEN("r3k2r/p1pp1pb1/bn2Qnp1/2qPN3/1p2P3/2N5/PPPBBPPP/R3K2R b KQkq - 3 2");

  EXPECT_EQ(perft(game_, state, 1), 5);
}

TEST_F(ChessPerftTest, Custom5) {
  auto state = chess::State::FromFEN("2kr3r/p1ppqpb1/bn2Qnp1/3PN3/1p2P3/2N5/PPPBBPPP/R3K2R b KQ - 3 2");

  EXPECT_EQ(perft(game_, state, 1), 44);
}

TEST_F(ChessPerftTest, Custom6) {
  auto state = chess::State::FromFEN("rnb2k1r/pp1Pbppp/2p5/q7/2B5/8/PPPQNnPP/RNB1K2R w KQ - 3 9");
...
TEST_F(ChessPerftTest, Custom8) {
  auto state = chess::State::FromFEN("rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8");

  EXPECT_EQ(perft(game_, state, 4), 2167396);
}

TEST_F(ChessPerftTest, Custom9) {
  auto state = chess::State::FromFEN("r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10");
...
TEST_F(ChessPerftTest, Kiwipete) {
  auto state = chess::State::FromFEN("r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1");

  EXPECT_EQ(perft(game_, state, 4), 4185552);
}

TEST_F(ChessPerftTest, Custom13) {
  auto state = chess::State::FromFEN("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1");

  EXPECT_EQ(perft(game_, state, 5), 720879);
}

TEST_F(ChessPerftTest, Custom14) {
  auto state = chess::State::FromFEN("r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1");
