...
std::string PrintMarkedBoard(State state, BoardPlane marker, std::string marker_color) {
  ...
  for (int y = board.GetHeight() - 1; y >= 0; --y) {
    ...
    for (int x = 0; x < board.GetWidth(); ++x) {
      ...
      if (piece == kEmptyPiece) {
        ...
        repr << s_color << (marked ? marker_color + "-" : " ") << s_color << s_piece << "\033[24m"
             << (marked ? marker_color + "-" : " ") << "\033[0m";
      }
    }
  }
